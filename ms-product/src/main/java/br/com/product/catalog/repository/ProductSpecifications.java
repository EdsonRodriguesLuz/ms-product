...
public class ProductSpecifications{
	...
	public List<Product> getProducts(String q, Double min_price, Double max_price) {
		...
		if (null != q && !"".equals(q)) {			
				Predicate sameName = criteriaBuilder.like(criteriaBuilder.lower(namePath), "%" + q.toLowerCase() + "%");
				Predicate sameDesc = criteriaBuilder.like(criteriaBuilder.lower(descriptionPath), "%" + q.toLowerCase() + "%");
